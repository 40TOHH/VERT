<script lang="ts">
	...
	// Drag and drop functionality - follows the same logic as main layout
	const dropFiles = (e: DragEvent) => {
		e.preventDefault();
		const oldLength = files.files.length;
		files.add(e.dataTransfer?.files);
		if (oldLength !== files.files.length) {
			goto(`/${getLocale()}/convert`); // Redirect to the convert page in the current locale
		}
	};

	const handleDrag = (e: DragEvent, drag: boolean) => {
		e.preventDefault();
		// In a real implementation, you might add visual feedback here
	};
</script>
	...
	>
		...
		<div class="max-w-2xl mx-auto">
			{#if files.files.length === 0}
				<button
					class="hover:scale-105 active:scale-100 duration-200 w-full h-full"
					onclick={() => {
						const uploader = document.createElement('input');
						uploader.type = 'file';
						uploader.multiple = true;
						uploader.accept = fromFormat?.mimeType || '*/*';
						uploader.onchange = (e) => {
							const target = e.target as HTMLInputElement;
							if (target.files) {
								const oldLength = files.files.length;
								files.add(target.files);
								if (oldLength !== files.files.length) {
									goto(`/${getLocale()}/convert`); // Redirect to the convert page in the current locale
								}
							}
				...
				>
					<div class="bg-panel flex justify-center items-center w-full h-full flex-col pointer-events-none p-3 rounded-2.5xl shadow-panel">
						<div class="w-16 h-16 bg-accent rounded-full flex items-center justify-center p-4">
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon lucide lucide-upload w-full h-full text-on-accent">
								<path d="M12 3v12"></path>
								<path d="m17 8-5-5-5 5"></path>
								<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
							</svg>
						</div>
						<h2 class="text-center text-2xl font-semibold mt-4">Перетащите или нажмите, чтобы конвертировать</h2>
						<p class="text-center text-muted mt-2">Поддерживаемый формат: {from.substring(1).toUpperCase()}</p>
					</div>
				</button>
			{:else}
				<ConversionPanel />
